...
export default class extends Component {
  ...
  // 获取列表
  getList = (page) => {
    let params = {...this.state.currentParams, page: page, pageSize: 10, position: '1'}
  ...
  getColumns() {
    ...
    const auth1 = !!authList.filter(v => +v === 1207).length // 编辑团队列表权限
    const auth2 = !!authList.filter(v => +v === 1208).length // 删除广告图权限
    if (auth1 || auth2) {
      list.push({
        title: '操作',
  ...
  onOkEdit = () => {
    ...
    this.refs.editModal.validateFields((err, values) => {
      ...
      let params = {...values, id: currentItem.id, position: '1'}
  ...
  render() {
    ...
    return (
      <div>
        {
          !!authList.filter(v => +v === 1206).length &&
