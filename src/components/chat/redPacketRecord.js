...
import { getRedPacketList, getAllCoinList } from '@/axios'
      ...
      return (
        <Form className="searchForm" layout="inline">
          ...
          <Form.Item label="发送对象">
            {getFieldDecorator('sendToType', {
              initialValue: 'all'
            })(
              <Select>
                <Option value="all">全部</Option>
                <Option value="groupChat">群</Option>
                <Option value="single">好友</Option>
              </Select>
            )}
          </Form.Item>
          <Form.Item label="发送对象ID">
...
export default class extends Component {
  ...
  getList = (pageOrSearch) => {
    ...
    const params = searchParams ? 
                   {sendToType: 'all', ...searchParams, page: currentPage, pageSize: 10} : 
                   {sendToType: 'all', ...this.state.currentParams, page: currentPage, pageSize: 10}
    this.setState({tableLoading: true, currentPage: currentPage})
    getRedPacketList(params).then(data => {
      this.setState({
  ...
  render() {
    ...
    return (
      <div>
        <div className="pageTitle">红包记录</div>
