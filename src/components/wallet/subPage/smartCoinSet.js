...
import { getCoinList, exchangeSmartSwitch } from '@/axios'
...
export default class extends Component {
  ...
  handleSwitch = (item, type) => {
    ...
    if (!authList.filter(v => +v === 3029).length) {
      message.warning('抱歉，您没有该权限！')
      return
    }
    if (type === 1) { // 是否开启AI智能宝充币开关
      let txt = item.smartRechargeable ? '关闭' : '开启'
      confirm({
        title: `是否${txt}${item.coinSymbol}的AI智能宝充币功能？`,
        onOk:() => {
          return new Promise((resolve, reject) => {
            exchangeSmartSwitch({id: item.id, rechargeable: !item.smartRechargeable}).then(() => {
              let list = [...this.state.tableList]
              list.forEach(v => {
                if (v.id === item.id) {
                  v.smartRechargeable = !item.smartRechargeable
                }
              })
    ...
    let columns = [
      {
        ...
        render: item => {
          return <Switch 
                  ...
                  checked={item.smartRechargeable} />
        }
      }
    ]
    return columns
