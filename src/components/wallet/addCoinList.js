...
import { getRechargeList, getAllCoinList, dlRecharge } from '@/axios'
...
export default class extends Component {
  ...
  getList = (pageOrSearch, pageSize2) => {
    ...
    getRechargeList(params).then(data => {
      this.setState({
  ...
  outExcel = () => {
    ...
    params.title = `充币记录（${titleArr.length ? titleArr.join('，') : '所有'}）`
    this.setState({excelLoading: true})
    dlRecharge(params).then(() => {
