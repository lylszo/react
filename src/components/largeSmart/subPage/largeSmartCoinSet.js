...
import { getCoinList, lsCoinOpenEdit } from '@/axios'
...
export default class extends Component {
  ...
  handleSwitch = (item, type) => {
    ...
    if (!authList.filter(v => +v === 9017).length) {
      message.warning('抱歉，您没有该权限！')
      return
    }
    if (type === 1) { // 是否开启LARGE智能宝
      let txt = item.isLarge ? '关闭' : '开启'
      confirm({
        title: `是否${txt}${item.coinSymbol}的LARGE智能宝？`,
        onOk:() => {
          return new Promise((resolve, reject) => {
            lsCoinOpenEdit({id: item.id, isLarge: !item.isLarge}).then(() => {
              let list = [...this.state.tableList]
              list.forEach(v => {
                if (v.id === item.id) {
                  v.isLarge = !item.isLarge
                }
              })
